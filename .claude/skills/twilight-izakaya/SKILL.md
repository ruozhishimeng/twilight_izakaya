...
> **当前定位（2026-04-17 起生效）**
>
> 本文档的**文学风格指导、叙事理念、写作原则**仍然有效且必须遵循：
> - 潜台词原则 (Show, Don't Tell)
> - 像素感白描 (Director's Lens)
> - 主角的克制、防御性开场、节奏结构
> - 内容独立性原则
> - short_story 详细角色设定写作规范
> - 人物故事节点解锁规范
> - Audio Schema Spec
>
> **以下内容已降级为历史参考**，不再作为格式权威来源：
> - YAML 节点结构示例中涉及 `type: dialogue/mixing/reward/transition/system/ending` 的部分
> - `reward.items[]` 格式（运行时已改为 `reward.details`）
> - chat/hidden 节点使用 `script_flow` 的示例（运行时已改为 `atmosphere_lines + content`）
> - `sfx` 对象数组格式（运行时已改为 `sfx.onEnter` 字符串数组）
> - `fade_in` / `fade_out` 字段名（运行时已改为 `fadeIn` / `fadeOut`）
>
> **格式契约以以下文档为准**：
> - [状态系统驱动内容规范.md](../../../docs/状态系统驱动内容规范.md) — 运行时字段契约
> - [状态系统驱动节点模板.md](../../../docs/状态系统驱动节点模板.md) — 最小可用脚手架
> - [twilight-yaml-converter SKILL.md](../twilight-yaml-converter/SKILL.md) — STAGE 4 产出规范
>
> **版本**：v1.1（历史版本，与《剧情文件规范文档 v1.0》对齐）
>
> **历史变更日志**：
> - 新增 `presentation` 演出系统、`audio` 音频系统
> - 统一 `reward` 结构为 `items[]` 数组格式（已废弃）
> - 统一 `branch_type` 为 `flavor | plot | hidden`
> - 新增 `diary_note` 正式规范
> - 明确 `observations.yaml` 和 `gallery.yaml` 结构
> - 新增 `transition`、`system` 节点类型（运行时不再使用节点 type 驱动）
