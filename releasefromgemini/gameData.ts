...
export interface GuestPhase {
  ...
  successNode?: any; // 保存 success 节点
  failNode?: any; // 保存 fail 节点
...
const nodesTeachingRaw = import.meta.glob('../assets/character/*/nodes_teaching_reward_challenge_ending.yaml', { eager: true, query: '?raw', import: 'default' });
...
const parseYamlMap = (rawMap: Record<string, unknown>) => {
  ...
  Object.keys(rawMap).forEach(key => {
    try {
      parsed[key] = yaml.load(rawMap[key] as string);
    } catch (e) {
  ...
  for (let i = 0; i < startNodes.length; i++) {
    ...
    while (currentNode && (!currentNode.player_options || currentNode.player_options.length === 0) && currentNode.next_node) {
      ...
      if (gameAction && (gameAction.type === 'enter_mixing' || gameAction.action === 'ENTER_MIXING_MODE')) {
        ...
        const mixingNode = findNodeForGuest(currentNode.next_node, charId, nodeMap);
        if (mixingNode) {
      ...
      currentNode = findNodeForGuest(currentNode.next_node, charId, nodeMap);
      if (currentNode) {
    ...
    while (currentNode && currentNode.player_options && currentNode.player_options.length > 0) {
      ...
      for (const opt of currentNode.player_options) {
        const replyNode = findNodeForGuest(opt.next_node, charId, nodeMap);
        ...
        while (currentReplyNode) {
          ...
          currentReplyNode = findNodeForGuest(currentReplyNode.next_node, charId, nodeMap);
        }
    ...
    if (currentNode) {
      ...
      if (successNodeId) {
        successNode = findNodeForGuest(successNodeId, charId, nodeMap);
        ...
        if (successNode && successNode.player_options && successNode.player_options.length > 0) {
          ...
          if (reactionNode.next_node) {
            rewardNode = findNodeForGuest(reactionNode.next_node, charId, nodeMap);
          }
        }
      }

      const failNodeId = currentNode.on_mixing_fail;
      if (failNodeId) failNode = findNodeForGuest(failNodeId, charId, nodeMap);
    }
    ...
    if (!failNode) {
      ...
      failNode = findNodeForGuest(failNodeId, charId, nodeMap);
      if (!failNode) {
    ...
    if (successNode) {
      ...
      while (curr) {
        ...
        curr = findNodeForGuest(curr.next_node, charId, nodeMap);
      }
    ...
    if (!successNode && startNode.player_options && startNode.player_options.length > 0) {
      const firstOptNode = findNodeForGuest(startNode.player_options[0].next_node, charId, nodeMap);
...
Object.keys(metaFiles).forEach(key => {
  ...
  const nodesMainKey = `../assets/character/${charId}/nodes_main.yaml`;
  const nodesTeachingKey = `../assets/character/${charId}/nodes_teaching_reward_challenge_ending.yaml`;
  const observationsKey = `../assets/character/${charId}/observations.yaml`;

  const nodesMain = nodesMainFiles[nodesMainKey];
  const nodesTeaching = nodesTeachingFiles[nodesTeachingKey];
  const observationsData = observationsFiles[observationsKey];

  const expressions: Record<string, string> = {};
  let image = '';
  
  Object.keys(imageFiles).forEach(imgKey => {
    if (imgKey.includes(`/character/${charId}/media/`)) {
      const fileName = imgKey.split('/').pop()?.replace('.png', '');
      if (fileName) {
  ...
  GUESTS.push({
    ...
    meta,
    nodeMap: new Map(allNodes.map(n => [n.event_id || n.id, n]))
  });
});
