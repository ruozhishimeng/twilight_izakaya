...
function ensureCharacter(
  source: Record<string, ParsedCharacterSource>,
...
function loadJsonDocument<T>(filePath: string) {
  return JSON.parse(fs.readFileSync(filePath, 'utf8')) as T;
}

export interface LoadContentFromFsOptions {
  repoRoot?: string;
}

/**
 * Loads authored content from disk without validating it or producing output.
 * CLI entry points and Node tests share this loader to avoid importing a script
 * with top-level side effects.
 */
export function loadContentSourceFromFs(
  options: LoadContentFromFsOptions = {},
): ParsedContentSource {
  ...
  const recipesPath = path.join(repoRoot, 'src', 'assets', 'recipes', 'recipes.json');
  ...
  return {
    ...
    recipes: loadJsonDocument<RecipesCatalog>(recipesPath),
    drinkImages: {},
  };
}
